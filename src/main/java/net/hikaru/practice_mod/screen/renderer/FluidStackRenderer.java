...
public class FluidStackRenderer implements IIngredientRenderer<FluidStack> {
    ...
    public void drawFluid(MatrixStack matrixStack, FluidStack fluidStack, int x, int y, int width, int height, long maxCapacity) {
        ...
        RenderSystem.setShaderTexture(0, FluidRenderHandlerRegistry.INSTANCE.get(fluidStack.getFluidVariant().getFluid())
                .getFluidSprites(MinecraftClient.getInstance().world, null, fluidStack.getFluidVariant().getFluid().getDefaultState())[0].getId());
    }
    ...
    public List<Text> getTooltip(FluidStack fluidStack, TooltipContext tooltipFlag) {
        ...
        if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
            MutableText amountString = Text.translatable("practice_mod.tooltip.liquid.amount.with.capacity",
                    nf.format(amount), nf.format(capacityMb));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
            MutableText amountString = Text.translatable("practice_mod.tooltip.liquid.amount",
                    nf.format(amount));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        }
