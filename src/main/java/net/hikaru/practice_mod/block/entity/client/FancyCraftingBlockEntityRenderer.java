package net.hikaru.practice_mod.block.entity.client;
...
public class FancyCraftingBlockEntityRenderer implements BlockEntityRenderer<FancyCraftingBlockEntity> {
    public FancyCraftingBlockEntityRenderer(BlockEntityRendererFactory.Context context) {

    }

    @Override
    public void render(FancyCraftingBlockEntity entity, float tickDelta, MatrixStack matrices,
                       VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ItemRenderer itemRenderer = MinecraftClient.getInstance().getItemRenderer();

        ItemStack itemStack = entity.getRenderStack();
        matrices.push();
        matrices.translate(0.5f, 1.005f, 0.5f);
        matrices.scale(0.2f, 0.2f, 0.2f);
        matrices.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(-90));

        switch (entity.getCachedState().get(FancyCraftingTableBlock.FACING)) {
